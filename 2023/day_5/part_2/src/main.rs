...
fn main() -> std::io::Result<()> {
    ...
    let (_, seed_data) = seeds_line.split_once(": ").unwrap();
    let seed_data: Vec<u64> = seed_data
    ...
    let seed = seed_data
        .chunks(2)
        .map(|x| {
            let (start, x) = x.split_first().unwrap();
            let (length, _) = x.split_first().unwrap();
            println!(
                "Running seed range: {} - {} ({})",
                start,
                start + length,
                    ...
                    for map in &maps {
                        for range in &map.ranges {
                            if x >= range.source && x < (range.source + range.length) {
                                x = range.destination + (x - range.source);
                                break;
                            }
                        }
                    }
